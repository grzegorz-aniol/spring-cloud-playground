...
#nohup 
java -Xmx500m -Xbootclasspath/p:alpn-boot.jar -jar ./bin/orders-http-service-0.0.1-SNAPSHOT.jar --spring.profiles.active=default,mysql,ssl --server.ssl.key-store=${TEST_KEYSTORE_NAME}.jks --server.ssl.key-store-password=${TEST_JKS_PASS} --server.ssl.key-password=${TEST_JKS_PASS} --server.host=$TEST_SERVER_HOST
